...
def PlotBarsDistribution(event,bars,time,nr_events):
    ...
    if(event == 'SN'):
        ...
        pmtCounts = pmtCounts / float(simulationTime)  # normalise as per microsecond
                                                        # for now
        # Extension: do the plot the Standard deviation
        # Plan: Save data as per interval of 5 us (for Ar39)
        simulationTime = np.max(time['time']) # in microseconds
        std_pmtCounts_Ar = np.zeros((int(simulationTime/5)+1,len(y),len(z)))
        for i in range(len(time)):
            std_pmtCounts_Ar[int(time['time'][i]/5)][int(time['pmt'][i]%len(y))][int(time['pmt'][i] / len(y))] += 1
    ...
    if(event == 'SN'):
        ...
        plt.xlabel('Z / cm')
        plt.ylabel('Y / cm')
        cbar.set_label('Mean photons per us', rotation = 270, labelpad=30, y=0.5, fontsize = 18)
        plt.title('Ar39 events')
        plt.savefig('Ar39_PMTDistrib_1us.png')
        plt.close()
        # Extension
        plt.pcolormesh(X,Y,3*std_pmtCounts_Ar,vmin=0,vmax=5.5)
        cbar = plt.colorbar()
        plt.xlabel('Z / cm')
        plt.ylabel('Y / cm')
        cbar.set_label('3*STD(photons) per 5 us', rotation = 270, labelpad=30, y=0.5, fontsize = 18)
        plt.title('Ar39 events')
